    ...
    io.on('connection', (socket) => {
        ...
        socket.on('host-start-quiz', async () => {
            try {
                ...
                await prisma_1.default.player.updateMany({
                ...
                await prisma_1.default.cheatLog.deleteMany({
                    where: { player: { roomId: room.id } }
                });
        ...
        socket.on('submit-answer', async (payload) => {
            try {
                ...
                const existingAnswer = await prisma_1.default.answer.findFirst({
                ...
                const totalPlayers = await prisma_1.default.player.count({
                    where: { roomId: room.id, isDisconnected: false }
                });
        ...
        socket.on('detect-tab-switch', async () => {
            try {
                ...
                await prisma_1.default.cheatLog.create({
                    data: {
                        playerId,
                        type: 'TAB_SWITCH',
            ...
            try {
                ...
                if (nextIndex < roomState.totalQuestions) {
                    ...
                    for (let i = 0; i < players.length; i++) {
                        ...
                        await prisma_1.default.score.create({
                            data: {
                                userId: p.userId,
                                username: p.name,
                                quizId: room.quizId,
...
const lockQuestion = async (io, roomCode) => {
    const roomState = activeRooms.get(roomCode);
    if (!roomState)
        return;
    if (roomState.questionTimer) {
